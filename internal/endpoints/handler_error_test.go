...
func Test_HandlerError_when_endpoint_returns_internal_error(t *testing.T) {
	assert := assert.New(t)
	enpoint := func(w http.ResponseWriter, r *http.Request) (interface{}, int, error) {
		return nil, 0, internalerrors.ErrInternal
	}
	handlerFunc := HandlerError(enpoint)
...
func Test_HandlerError_when_endpoint_returns_obj_and_status(t *testing.T) {
	...
	objectExpected := BodyForTest{123}
	enpoint := func(w http.ResponseWriter, r *http.Request) (interface{}, int, error) {
		return objectExpected, http.StatusCreated, nil
	}
	handlerFunc := HandlerError(enpoint)
	req, _ := http.NewRequest("GET", "/", nil)
	res := httptest.NewRecorder()
	...
	assert.Equal(objectExpected, objReturned)
}
